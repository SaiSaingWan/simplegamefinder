...
// Screens - Ensure these paths match your folder structure
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn && userId) {
      // 1. Listen for Profile Changes (Name, Age, etc.)
      ...
      // 2. Listen for Level Progress (Count "Completed" games)
      const q = query(
      ...
      unsubLevel = onSnapshot(q, (snapshot) => {
        ...
        // Formula: Start at level 1, +1 level for every 2 games completed
        ...
        <div>
          ...
          <div style={webStyles.userBadge}>
            <div style={webStyles.avatarContainer}>
               <div style={webStyles.avatarSmall}>{userName.substring(0, 1).toUpperCase()}</div>
               <div style={webStyles.levelCircle}>{userLevel}</div>
            </div>
            <div>
              <p style={webStyles.sidebarName}>{userName}</p>
              <p style={webStyles.levelText}>Tier {userLevel} Member</p>
            </div>
          </div>
...
const webStyles = {
  ...
  logo: { fontSize: '24px', fontWeight: 'bold', color: '#00D1FF', marginBottom: '25px' },
  ...
  avatarSmall: { width: '40px', height: '40px', backgroundColor: '#00D1FF', color: '#000', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '18px' },
