...
const SavedGamesScreen = ({ onSelectGame }) => {
  ...
  // 2. Get the real UID from the logged-in user
  ...
  useEffect(() => {
    ...
    // 3. Query using the REAL currentUserId
    const q = query(
  ...
  if (!currentUserId) return <div style={styles.loader}>Please log in to see your library.</div>;
  ...
  return (
    <div style={styles.container}>
      ...
      {games.length === 0 ? (
        <p style={{color: '#64748B'}}>No games saved yet. Go find some!</p>
      ) : (
...
const styles = {
  ...
  loader: { textAlign: 'center', marginTop: '100px', color: '#fff' }
};
